...
class Variational_Autoencoder():
    def __init__(self, sess, build_encoder, build_decoder, checkpoint_name = 'vae_feats_checkpoint',
        batch_size = 100, z_dim = 20, img_dim = 256 * 8 * 8, dataset = 'mnist',
        learning_rate = 0.001, num_epochs = 5,load = False,load_file = None,
        checkpoint_dir = '../notebook/checkpoints/',layer = 4):
        """
        Inputs:
        ...
        if dataset == 'mnist' and load == False:
            ...
            temp_sess = tf.InteractiveSession()
            allimages = self.mnist.train.images.reshape(55000,28,28,1)
            mean_img = np.mean(allimages,axis = 0)
            mean_img = convert_images(mean_img).eval()
            self.mean_img = np.rollaxis(mean_img,2,0)
            temp_sess.close()
        ...
        if self.load:
            ...
            for epoch in xrange(self.num_epochs): 
                ...
                for b in xrange(num_batches):
                    ...
                    print 'Batch {} loss: {}'.format(b + 1, loss_value) 
    ...
    def input(self):
        ...
        if self.dataset == 'mnist':
            ...
            batch_images = batch_images.reshape((self.batch_size,28,28,1))
            ...
            feats = image_to_feats(rgb_images,self.mean_img,self.pretrained_model,self.layer)
            feats = feats.reshape((self.batch_size,-1))
            return feats
