...
class CVAEGAN():
    def __init__(self, sess, build_encoder, build_decoder, build_discriminator, checkpoint_name = 'vae_checkpoint',
        ...
        learning_rate = 0.001, lr_decay=0.95, lr_decay_freq=1000, num_epochs = 5,load = False,load_file = None, alpha = 10., beta = 0.0001, gamma=1., x_width=28,summaries_dir = './', checkpoint_dir = '../notebook/checkpoints/'):
        """
        Inputs:
        ...
        self.summary_fool_loss = tf.scalar_summary('loss for fooling the discriminator', self.xentropy_fresh_mean)
        ...
        for epoch in xrange(self.num_epochs):
            ...
            for b in xrange(num_batches):
                ...
                _, vae_loss, x_mean, x_fresh,encoder_loss,decoder_loss,fake_loss = self.sess.run(
                    ...
                    self.enc_loss,self.decoder_loss,self.xentropy_fresh_mean],
    ...
    def build_vae(self):
        ...
        with tf.variable_scope('D'):
            ...
            self.dis_real,self.lth_layer_real = self.build_discriminator(self.x,
                x_width=self.x_width,train_phase = True)

            # # reconstruction
            self.dis_fake_recon,self.lth_layer_recon = self.build_discriminator(self.x_mean, reuse=True,
                x_width=self.x_width,train_phase = True)

            # fresh generation
            self.dis_fake_fresh, self.lth_layer_fresh = self.build_discriminator(self.x_fresh, reuse=True,
                x_width=self.x_width,train_phase = True) 
        ...
        vae_loss = self.alpha * self.enc_loss + self.beta * self.decoder_loss - self.gamma * self.xentropy_fresh_mean 
