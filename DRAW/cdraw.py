...
class DRAW():
    ...
    def train(self, train_itrs = 1000, batch_size = 100, print_itrs = 1000, learning_rate = 1e-4, load = False, save = False):
        ...
        if load:
            self.saver.restore(sess, "cdraw.ckpt") # to restore from model, uncomment this line
        
        for i in range(self.train_itrs):

            # batch_eps = np.random.randn(self.T, self.batch_size, self.z_size)
	    xtrain, y_train = train_data.next_batch(self.batch_size) # xtrain is (batch_size x img_size)
            
            self.feed_dict = {self.x : xtrain, self.info: y_train}
        ...
        if save:
            self.saver.save(sess,'cdraw.ckpt')
