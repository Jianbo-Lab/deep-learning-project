...
def build_encoder(images,z_dim):
    ...
    h0 = tf.nn.relu(bn0(conv2d(images, filter_dim, name='en_conv0')))

    h1 = tf.nn.relu(bn1(conv2d(h0, filter_dim * 2, name='en_conv1')))

    h2 = tf.nn.relu(bn2(linear(tf.reshape(h1, [batch_size, -1]), hidden_dim, 'en_fc3')))
...
def build_decoder(z,img_dim):
    ...
    h0 = tf.reshape(h0, [-1, s4, s4, filter_dim * 2])
    h0 = tf.nn.relu(bn0(h0))

    h1 = deconv2d(h0, [batch_size, s2, s2, filter_dim], 
        name='de_deconv1')
