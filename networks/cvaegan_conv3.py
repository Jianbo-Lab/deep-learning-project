...
class Encoder:
    ...
    def __call__(self, x, y, z_dim, batch_size=100,train_phase = True):
        ...
        net = slim.conv2d(net, self.x_width, [5,5], stride=2,
            normalizer_fn=slim.batch_norm,
            normalizer_params={'is_training':train_phase,
                'scale':True,
                'updates_collections':None,
                'scope':'enc_bn0'}) 
        net = slim.conv2d(net, self.x_width * 2, [5,5], stride=2,
            normalizer_fn=slim.batch_norm,
            normalizer_params={'is_training':train_phase,
                'scale':True,
                'updates_collections':None,
                'scope':'enc_bn1'}) 
        net = slim.conv2d(net, self.x_width * 4, [5,5], stride=2,
            normalizer_fn=slim.batch_norm,
            normalizer_params={'is_training':train_phase,
                'scale':True,
                'updates_collections':None,
...
class Decoder:
    ...
    def __call__(self, z, x_dim, batch_size=100, reuse=None,train_phase = True):
        ...
        if self.layer == 4:
            ...
            net = z
            net = slim.fully_connected(net, network_size*network_size*256, reuse=reuse, scope='dec_fc0',
                normalizer_fn=slim.batch_norm,
                normalizer_params={'reuse':reuse,
                    'is_training':train_phase,
                    'scale':True,
